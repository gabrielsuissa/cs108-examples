...
class CreateQuoteView(CreateView):
    '''A view to create a new quote and save it in the database.'''
...
class DeleteQuoteView(DeleteView):
    '''A view to delete a quote and save it in the database.'''

    template_name = "quotes/delete_quote.html"
    queryset = Quote.objects.all()
    #success_url = '../../all' # what to do when deleting
...
def add_image(request, pk): 
    ...
    if form.is_valid():
        ...
        image = form.save(commit=False) # create the object but not save it
        image.person = person
